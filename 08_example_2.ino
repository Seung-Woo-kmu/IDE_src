...
// turn on the LED if the distance is between dist_min and dist_max
  ...
  analogWrite(PIN_LED, int(255 - 255/100*(dist_raw-100)));
  if (200 <= dist_raw && dist_raw <= 300);
  analogWrite(PIN_LED, int(255/100*(dist_raw-200)));
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) reading = Reading;
  
  if (dist_min <= reading && reading <= dist_max) Reading = reading;
